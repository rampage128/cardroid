...
public final class CanReader {
    ...
    private static class CanConfig {
        ...
        private void registerCanIds(CanInteractable interactable) {
            for(int i = 0; i < this.packetDescriptors.size(); i++) {
                Log.e(this.getClass().getSimpleName(), "Register can id " + String.format("%04X", this.packetDescriptors.valueAt(i).getCanId()));
                interactable.registerCanId(this.packetDescriptors.valueAt(i));
            }
        }
